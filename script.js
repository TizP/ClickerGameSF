...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Constants ---
    const GAME_VERSION = "1.15"; // Current version
    const SAVE_KEY = `salesforcePipelineSaveData_v${GAME_VERSION}`; // Use version in save key
    const TICK_INTERVAL_MS = 100; // Core game logic loop (10 times/sec)
    const DISPLAY_UPDATE_INTERVAL_MS = 250; // Update resource display (4 times/sec)
    const BUTTON_UPDATE_INTERVAL_MS = 1000; // Update button states (1 time/sec)
    ...
    // musicShouldBePlaying defined globally
    ...
    function calculateDerivedStats() {
        ...
        baseCAR += gameState.custUpgradeBonusCAR || 0; baseCVR += gameState.custUpgradeBonusCVR || 0;
        baseCVR *= (gameState.cvrMultiplierBonus || 1.0); baseCVR *= (gameState.cvrCustomerMultiplier || 1.0); // Apply both CVR multipliers

        // 4. Assign final values
        leadsPerSecond = Math.max(0, newLPS); opportunitiesPerSecond = Math.max(0, newOPS);
    ...
    // *** UPDATED TO MATCH CLEANED UP HTML STATS ***
    function updateDisplay() {
        try {
             const coreElements = [domElements.leads, domElements.opportunities, domElements.customers, domElements.money, domElements.lps, domElements.ops, domElements.mps, domElements['leads-per-click'], domElements['opps-per-click'], domElements.car, domElements['success-chance'], domElements.cvr, domElements['cust-cost']];
             if (coreElements.some(el => !el)) return; // Skip if core elements missing
            ...
            domElements.money.textContent = '$' + formatMoney(gameState.money); // Add $ prefix

            // Use formatNumber as units are now in labels
            domElements.lps.textContent = formatNumber(leadsPerSecond);
            domElements.ops.textContent = formatNumber(opportunitiesPerSecond);
            domElements.mps.textContent = '$' + formatMoney(moneyPerSecond); // Add $ prefix
            ...
            if (domElements['lead-click-base-p']) { const bonus = leadsPerSecond * (gameState.leadClickPercentBonus || 0); domElements['lead-click-base-p'].title = `Base: ${gameState.leadsPerClick}. Bonus: +${formatPercent(gameState.leadClickPercentBonus, 1)} L/S (${formatPerSecond(bonus,'L').replace('/Sec','')}).`; }
            if (domElements['opp-click-base-p']) { const bonus = opportunitiesPerSecond * (gameState.oppClickPercentBonus || 0); domElements['opp-click-base-p'].title = `Base: ${gameState.opportunitiesPerClick}. Bonus: +${formatPercent(gameState.oppClickPercentBonus, 1)} O/S (${formatPerSecond(bonus,'O').replace('/Sec','')}).`; }

            // Update acquisition/value stats without units
            domElements.car.textContent = formatCAR(customerAcquisitionRate);
            domElements['success-chance'].textContent = (gameState.acquisitionSuccessChance * 100).toFixed(1); // Display as number, % is in label
            domElements.cvr.textContent = formatRateMoney(customerValueRate); // Display as number, units in label
            domElements['cust-cost'].textContent = formatNumber(getCurrentCustomerCost()); // Display as number, L&O in label
