...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Constants ---
    const GAME_VERSION = "1.18"; // Updated for powerup frequency/display text
    const SAVE_KEY = `salesforcePipelineSaveData_v${GAME_VERSION}`;
    const TICK_INTERVAL_MS = 100;
    const DISPLAY_UPDATE_INTERVAL_MS = 250; // Interval for updating stats AND powerup timer
    ...
    // --- Powerup Constants ---
    const POWERUP_SPAWN_INTERVAL_MS = 10000; // Check every 10 seconds
    const POWERUP_CHANCE_PER_INTERVAL = 0.0333; // Approx 1 spawn every 300s (5 minutes)
    const POWERUP_FALL_DURATION_MS = 8000; // How long it takes to fall (8 seconds)
    const POWERUP_SFX_CLICK_ID = 'sfx-powerup-click';
    const POWERUP_TOKEN_SIZE = 90; // px - Match CSS width/height
    ...
    let currentPowerupToken = null; // Track the single active token element
    ...
    function cacheDOMElements() {
        ...
        // Check specific required elements quietly, log if needed for debug
        ...
        // if (!domElements['active-powerup-display']) { console.warn(`Active Powerup Display area not found.`); }
        // if (!domElements['stat-powerups-clicked']) { console.warn(`Stat element 'stat-powerups-clicked' not found.`); }
    ...
    function calculateDerivedStats() {
        let newLPS = 0, newOPS = 0, baseCAR = 0.1, baseCVR = 1.0;
        const globalEff = gameState.buildingEfficiencyMultiplier || 1.0;
        const custGlobalMult = gameState.custGlobalMultiplier || 1.0;
        // 1. Base production
        for (const id in buildingsConfig) { const cfg = buildingsConfig[id], count = gameState.buildings[id]?.count || 0; if (count > 0) { let bLPS = cfg.baseLPS || 0, bOPS = cfg.baseOPS || 0, fLPS = 0, fOPS = 0, pLPS = 1.0, pOPS = 1.0, mLPS = 1.0, mOPS = 1.0; for (const upId in upgradesConfig) { const uCfg = upgradesConfig[upId]; if (gameState.upgrades[upId]?.purchased && uCfg.targetBuilding === id) { if (uCfg.flatBonusLPS) fLPS += uCfg.flatBonusLPS; if (uCfg.flatBonusOPS) fOPS += uCfg.flatBonusOPS; if (uCfg.percentBonusLPS) pLPS += uCfg.percentBonusLPS; if (uCfg.percentBonusOPS) pOPS += uCfg.percentBonusOPS; if (uCfg.multiplierBonusLPS) mLPS *= uCfg.multiplierBonusLPS; if (uCfg.multiplierBonusOPS) mOPS *= uCfg.multiplierBonusOPS; } } let finLPS = (bLPS + fLPS) * pLPS * mLPS, finOPS = (bOPS + fOPS) * pOPS * mOPS; newLPS += finLPS * count * globalEff * custGlobalMult; newOPS += finOPS * count * globalEff * custGlobalMult; } }
        // 2. Flexible Workflow Balancing
        if (gameState.flexibleWorkflowActive) { if (Math.abs(newLPS - newOPS) < FLEX_WORKFLOW_EQUALITY_THRESHOLD && (newLPS > 0 || newOPS > 0)) { gameState.flexibleWorkflowActive = false; } else if (newLPS !== newOPS) { const high = Math.max(newLPS, newOPS), transfer = high * 0.5; if (newLPS > newOPS) { newLPS -= transfer; newOPS += transfer; } else { newOPS -= transfer; newLPS += transfer; } newLPS = Math.max(0, newLPS); newOPS = Math.max(0, newOPS); } }
        // 3. Apply Production Boost (Powerup)
        const prodBoost = gameState.activeBoosts?.['prodBoost']; if (prodBoost) { const mult = 1.0 + prodBoost.magnitude; newLPS *= mult; newOPS *= mult; }
        ...
        // 6. Assign final rate values
        leadsPerSecond = Math.max(0, newLPS); opportunitiesPerSecond = Math.max(0, newOPS); customerAcquisitionRate = Math.max(0, baseCAR); customerValueRate = Math.max(0, baseCVR);
        // 7. Calculate Money/Second (incorporating powerup)
        let finalMPS = (gameState.customers || 0) * customerValueRate; const moneyBoost = gameState.activeBoosts?.['moneyBoost']; if (moneyBoost) { finalMPS *= (1.0 + moneyBoost.magnitude); } moneyPerSecond = finalMPS;
    }

    // --- Display Update Functions ---
    function updateDisplay() { try { const core = [domElements.leads, domElements.opportunities, domElements.customers, domElements.money, domElements.lps, domElements.ops, domElements.mps, domElements['leads-per-click'], domElements['opps-per-click'], domElements.car, domElements['success-chance'], domElements.cvr, domElements['cust-cost']]; if (core.some(el => !el)) return; domElements.leads.textContent = formatNumber(gameState.leads); domElements.opportunities.textContent = formatNumber(gameState.opportunities); domElements.customers.textContent = formatNumber(gameState.customers); domElements.money.textContent = '$' + formatMoney(gameState.money); domElements.lps.textContent = formatNumber(leadsPerSecond); domElements.ops.textContent = formatNumber(opportunitiesPerSecond); domElements.mps.textContent = '$' + formatMoney(moneyPerSecond); domElements['leads-per-click'].textContent = formatNumber(gameState.leadsPerClick); domElements['opps-per-click'].textContent = formatNumber(gameState.opportunitiesPerClick); if (domElements['lead-click-base-p']) { const b = leadsPerSecond * (gameState.leadClickPercentBonus || 0); domElements['lead-click-base-p'].title = `Base:${gameState.leadsPerClick}. Bonus:+${formatPercent(gameState.leadClickPercentBonus, 1)} L/S (${formatNumber(b)}).`; } if (domElements['opp-click-base-p']) { const b = opportunitiesPerSecond * (gameState.oppClickPercentBonus || 0); domElements['opp-click-base-p'].title = `Base:${gameState.opportunitiesPerClick}. Bonus:+${formatPercent(gameState.oppClickPercentBonus, 1)} O/S (${formatNumber(b)}).`; } domElements.car.textContent = formatCAR(customerAcquisitionRate); domElements['success-chance'].textContent = (gameState.acquisitionSuccessChance * 100).toFixed(1); domElements.cvr.textContent = formatRateMoney(customerValueRate); domElements['cust-cost'].textContent = formatNumber(getCurrentCustomerCost()); updateActivePowerupDisplay(); } catch (e) { console.error("Err updateDisplay:", e); } }
    ...
    // --- Sound Effect Helper ---
    function playSoundEffect(audioElement) { if (audioElement && audioElement.readyState >= 2) { audioElement.currentTime = 0; audioElement.play().catch(()=>{}); } }
    ...
    function setVolume(val=null) { const music=domElements['background-music'], slider=domElements['volume-slider'], sfx=domElements['sfx-purchase'], pwrSfx=domElements[POWERUP_SFX_CLICK_ID]; if(music&&slider){ let vol=val??parseFloat(slider.value); if(isNaN(vol))vol=0.1; slider.value=vol; music.volume=vol; const sfxVol = Math.min(1, vol * 1.5); if(sfx)sfx.volume=sfxVol; if(pwrSfx) pwrSfx.volume = sfxVol; }}
    ...
    function createPowerupToken(data) { const area = domElements['powerup-spawn-area']; if (!area || currentPowerupToken) return; const token = document.createElement('div'); currentPowerupToken = token; token.classList.add('powerup-token'); token.style.backgroundImage = `url('resources/img/${data.image}')`; token.title = `${data.name} (${data.description}) - Click Me!`; // Add description to title
        token.powerupData = data; const areaW = area.offsetWidth; token.style.left = `${Math.random() * (areaW - POWERUP_TOKEN_SIZE)}px`; token.style.top = '-100px'; const clickHandler = (e) => { e.stopPropagation(); if (!token.parentNode) return; gameState.totalPowerupsClicked++; applyBoost(token.powerupData); playSoundEffect(domElements[POWERUP_SFX_CLICK_ID]); removeActivePowerupToken(token); }; const animEndHandler = () => removeActivePowerupToken(token); token.addEventListener('click', clickHandler); token.addEventListener('animationend', animEndHandler, { once: true }); token.style.animation = `fallAnimation ${POWERUP_FALL_DURATION_MS}ms linear`; area.appendChild(token); }
    function removeActivePowerupToken(tokenElement = null) { const tokenToRemove = tokenElement || currentPowerupToken; if (tokenToRemove && tokenToRemove.parentNode) { tokenToRemove.remove(); } if (tokenToRemove === currentPowerupToken) { currentPowerupToken = null; } } // Clear tracking
    function applyBoost(data) { const id = data.id, dur = data.duration, mag = data.magnitude; if (gameState.powerupTimeouts[id]) clearTimeout(gameState.powerupTimeouts[id]); // Get description from powerupTypes based on id
        const powerupConfig = powerupTypes.find(p => p.id === id); const description = powerupConfig ? powerupConfig.description : ''; gameState.activeBoosts[id] = { endTime: Date.now() + dur, magnitude: mag, name: data.name, description: description // Store description
        }; gameState.powerupTimeouts[id] = setTimeout(() => removeBoost(id), dur); calculateDerivedStats(); updateDisplay(); updateButtonStates(); }
    ...
    function updateActivePowerupDisplay() {
        ...
        if (activeIds.length === 0) {
            displayEl.innerHTML = ''; // Use innerHTML to clear potential <br>
            return;
        }

        // Display only the first active boost if multiple somehow overlap (unlikely with current logic)
        ...
        if (remainingTimeMs <= 0) {
            displayEl.innerHTML = '';
            // Optionally call removeBoost here if timer somehow failed, though calculateDerivedStats/updateDisplay should handle it
        } else {
            const remainingSeconds = (remainingTimeMs / 1000).toFixed(1);
            // Use innerHTML to allow for line break
    ...
    function restartBoostTimersOnLoad() { const now = Date.now(); gameState.powerupTimeouts = {}; for (const id in gameState.activeBoosts) { const boost = gameState.activeBoosts[id]; const remaining = boost.endTime - now; // Ensure description is loaded from config if missing in save (backward compatibility)
        if (!boost.description) { const powerupConfig = powerupTypes.find(p => p.id === id); boost.description = powerupConfig ? powerupConfig.description : ''; } if (remaining > 0) { gameState.powerupTimeouts[id] = setTimeout(() => removeBoost(id), remaining); } else { delete gameState.activeBoosts[id]; } } calculateDerivedStats(); }
    ...
    function loadGame() { const json=localStorage.getItem(SAVE_KEY); gameState=getDefaultGameState(); let ok=false; if(json){ try { const data=JSON.parse(json); for(const key in gameState){ if(key==='powerupTimeouts')continue; if(data.hasOwnProperty(key)){ if(key==='activeBoosts'){gameState.activeBoosts=(typeof data.activeBoosts==='object'&&data.activeBoosts!==null)?data.activeBoosts:{};} else if((key==='buildings'||key==='upgrades')&&typeof data[key]==='object'&&data[key]!==null){for(const id in data[key]){if((key==='buildings'&&buildingsConfig[id])||(key==='upgrades'&&upgradesConfig[id])){if(!gameState[key][id])gameState[key][id]={}; gameState[key][id]={...gameState[key][id], ...data[key][id]};}}} else if(typeof gameState[key]!=='object'||gameState[key]===null){gameState[key]=data[key];}}} initializeStructureState(gameState, false); restartBoostTimersOnLoad(); // This now also handles adding descriptions if missing
            console.log("Save Loaded."); displaySaveStatus("Save loaded."); ok=true; } catch(e){ console.error("Load error:", e); displaySaveStatus("Load error! Resetting.", 5000); localStorage.removeItem(SAVE_KEY); gameState=getDefaultGameState(); } } else { console.log("No save found."); } calculateDerivedStats(); return ok; }
    ...
    // --- Initialization ---
    function initializeGame() { console.log(`--- Initializing Game v${GAME_VERSION} ---`); try { cacheDOMElements(); if (domElements['game-version']) { domElements['game-version'].textContent = `v${GAME_VERSION}`; } } catch (e) { console.error("DOM cache error:", e); alert("Fatal UI Init Error."); return; } loadGame(); updateDisplay(); updateButtonStates(); setupEventListeners(); if (domElements['background-music']&&domElements['volume-slider']){setVolume(0.1);loadTrack(0,false);} else {console.warn("Music elements missing.");} startGameIntervals(); if(!isGameWon&&gameState.money>=WIN_AMOUNT){console.log("Loaded won state.");isGameWon=true;isGamePaused=true; stopPowerupSpawning(); updateButtonStates();updateAcquisitionButtonVisuals();updateFlexibleWorkflowToggleButtonVisuals(); removeActivePowerupToken(); showModal(domElements['win-modal']);} console.log("--- Game Initialized ---"); }
